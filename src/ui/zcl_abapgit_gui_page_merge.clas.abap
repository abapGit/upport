...
CLASS ZCL_ABAPGIT_GUI_PAGE_MERGE IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    mo_merge = NEW #( io_repo = io_repo iv_source_branch = iv_source ).
    mo_merge->run( ).
    ...
    CASE iv_action.
      WHEN c_actions-merge.
        ...
        IF mo_repo->get_local_settings( )-code_inspector_check_variant IS NOT INITIAL.
          ...
          ei_page = NEW zcl_abapgit_gui_page_code_insp( io_repo = mo_repo io_stage = mo_merge->get_result( )-stage ).

        ELSE.

          ei_page = NEW zcl_abapgit_gui_page_commit( io_repo = mo_repo io_stage = mo_merge->get_result( )-stage ).
      ...
      WHEN c_actions-res_conflicts.
        ...
        ei_page = NEW zcl_abapgit_gui_page_merge_res( io_repo = mo_repo io_merge_page = me io_merge = mo_merge ).
        ev_state = zcl_abapgit_gui=>c_event_state-new_page.
